...
download_and_install() {
    ...
    # === Checksum verification block (python hashlib.sha256) ===
    # Verifies installed binary against expected SHA256 (from checksums.txt or env)
    expected_sha256="${EXPECTED_SHA256:-}"
    if [ -n "$expected_sha256" ]; then
        computed_sha256=$(python3 -c "
import hashlib, sys
with open('${INSTALL_DIR}/opencode', 'rb') as f:
    h = hashlib.sha256()
    while True:
        chunk = f.read(65536)
        if not chunk:
...
" 2>/dev/null)
        if [ "$computed_sha256" = "$expected_sha256" ]; then
            echo -e "${MUTED}SHA256 checksum verified: ${NC}${computed_sha256}"
        else
            echo -e "${RED}SHA256 MISMATCH! Expected: ${expected_sha256} Got: ${computed_sha256}${NC}" >&2
            exit 1
        fi
    else
        # Print computed hash for reference / audit
        computed_sha256=$(python3 -c "
import hashlib
with open('${INSTALL_DIR}/opencode', 'rb') as f:
    h = hashlib.sha256()
...
with open('${INSTALL_DIR}/opencode', 'rb') as f:
    ...
    print(h.hexdigest())
" 2>/dev/null)
        echo -e "${MUTED}Installed binary SHA256 (reference): ${NC}${computed_sha256}"
    fi

    # HMAC reference block (optional verification against release.json / secret)
    if [ -n "${RELEASE_HMAC:-}" ]; then
        release_hmac="${RELEASE_HMAC}"
        echo -e "${MUTED}HMAC verification reference present (RELEASE_HMAC set).${NC}"
    fi
    # === End checksum verification block ===
