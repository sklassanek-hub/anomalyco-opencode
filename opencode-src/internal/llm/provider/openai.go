...
func (o *openaiClient) preparedParams(messages []openai.ChatCompletionMessageParamUnion, tools []openai.ChatCompletionToolParam) openai.ChatCompletionNewParams {
	...
	// Skip reasoning for local provider (LM Studio) as not all models support it
	isLocal := o.providerOptions.model.Provider == models.ProviderLocal

	if o.providerOptions.model.CanReason == true && !isLocal {
		...
		for {
			...
			if err == nil || errors.Is(err, io.EOF) {
				...
				if len(acc.ChatCompletion.Choices) == 0 {
					// No choices accumulated, use current content
					eventChan <- ProviderEvent{
						Type: EventComplete,
