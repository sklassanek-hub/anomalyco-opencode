...
- For a new agent in this repository, prefer `вњЁ` over `рџ“љ` because the change adds a new catalog capability rather than only updating existing documentation
...
|-------------|----------------|----------------|-------------|
| Feature | `feature/JIRA-214-add-sso-login` | `вњЁ JIRA-214: add SSO login flow` | New product or platform capability |
| Bug Fix | `bugfix/JIRA-315-fix-token-refresh` | `рџђ› JIRA-315: fix token refresh race` | Non-production-critical defect work |
| Hotfix | `hotfix/JIRA-411-patch-auth-bypass` | `рџђ› JIRA-411: patch auth bypass check` | Production-critical fix from `main` |
| Refactor | `feature/JIRA-522-refactor-audit-service` | `в™»пёЏ JIRA-522: refactor audit service boundaries` | Structural cleanup tied to a tracked task |
| Docs | `feature/JIRA-623-document-api-errors` | `рџ“љ JIRA-623: document API error catalog` | Documentation work with a Jira task |
| Tests | `bugfix/JIRA-724-cover-session-timeouts` | `рџ§Є JIRA-724: add session timeout regression tests` | Test-only change tied to a tracked defect or feature |
| Config | `feature/JIRA-811-add-ci-policy-check` | `рџ”§ JIRA-811: add branch policy validation` | Configuration or workflow policy changes |
| Dependencies | `bugfix/JIRA-902-upgrade-actions` | `рџ“¦ JIRA-902: upgrade GitHub Actions versions` | Dependency or platform upgrades |
...
- Repository-specific default: use `вњЁ` when adding a brand-new agent because Gitmoji defines it for new features; use `рџ“љ` only when the change is limited to documentation updates around existing agents or contribution docs
...
commit_regex='^(рџљЂ|вњЁ|рџђ›|в™»пёЏ|рџ“љ|рџ§Є|рџ’„|рџ”§|рџ“¦) [A-Z]+-[0-9]+: .+$'
...
## Planned Commits
1. рџђ› JIRA-315: fix refresh token race in auth service
2. рџ§Є JIRA-315: add concurrent refresh regression tests
3. рџ“љ JIRA-315: document token refresh failure modes
