...
def compute_psi(expected: pd.Series, actual: pd.Series, bins: int = 10) -> float:
    ...
    Interpretation:
      < 0.10  в†’ No significant shift (green)
      0.10вЂ“0.25 в†’ Moderate shift, investigation recommended (amber)
      >= 0.25 в†’ Significant shift, action required (red)
    """
...
def pdp_interaction(
    ...
    ax.set_title(f"PDP Interaction - {feature_pair[0]} Г— {feature_pair[1]}")
    fig.tight_layout()
    fig.savefig(
    ...
    for var in variables:
        for period in periods[1:]:
            ...
            results.append({
                ...
                "period": period,
                "psi": psi,
                "flag": "рџ”ґ" if psi >= psi_threshold else (
                    "рџџЎ" if psi >= 0.10 else "рџџў"
                ),
            })
