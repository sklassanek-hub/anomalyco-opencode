...
```sql
-- вќЊ Bad: N+1 query pattern
SELECT * FROM posts WHERE user_id = 123;
-- Then for each post:
SELECT * FROM comments WHERE post_id = ?;

-- вњ… Good: Single query with JOIN
EXPLAIN ANALYZE
SELECT 
...
```typescript
// вќЊ Bad: N+1 in application code
...
// вњ… Good: Single query with aggregation
...
4. **Safe Migrations**
```sql
-- вњ… Good: Reversible migration with no locks
BEGIN;
...
-- вќЊ Bad: Locks table during migration
