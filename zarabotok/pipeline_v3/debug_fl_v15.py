...
with sync_playwright() as p:
    ...
    # Enable request/response logging - capture ALL requests
    page.on('response', lambda r: print(f'RESPONSE: {r.status} {r.url}') if 'offer' in r.url or 'message' in r.url or 'bid' in r.url or 'answer' in r.url or 'project' in r.url or 'offer' in r.url or 'xajax' in r.url or 'answer' in r.url or 'reply' in r.url else None)
    page.on('request', lambda r: print(f'REQUEST: {r.method} {r.url}') if 'offer' in r.url or 'message' in r.url or 'bid' in r.url or 'answer' in r.url or 'project' in r.url or 'offer' in r.url or 'xajax' in r.url or 'answer' in r.url else None)
    ...
    if btn:
        ...
        # Wait for response - try to catch the AJAX request
        ...
        # Check for any new elements with "offer" in class/id
        ...
        # Check for any new elements after click
        print('Checking all elements with "offer" in class/id...')
        offer_elements = page.query_selector_all('[class*="offer"], [id*="offer"]')
        ...
        for i, el in enumerate(offer_elements[:10]):
            ...
            ta = el.query_selector('textarea')
            if ta:
                print(f'  Found textarea: name={ta.get_attribute("name")}, id={ta.get_attribute("id")}')
        
        # Check for any modal/overlay
        modals = page.query_selector_all('.modal, .ui-modal, .ui-dialog, [role=dialog], .ui-overlay')
        print(f'Modals/overlays: {len(modals)}')
        for i, modal in enumerate(modals):
            print(f'  {i}: id={modal.get_attribute("id")}, class={modal.get_attribute("class")}')
        ...
        # Check for any button with "Отправить" text
