...
with sync_playwright() as p:
    ...
    if btn_open:
        ...
        for i, el in enumerate(offer_elements[:10]):
            print(f'  {i}: tag={el.evaluate("el => el.tagName")}, id={el.get_attribute("id")}, class={el.get_attribute("class")}')
            # Check for textarea inside
        ...
        modals = page.query_selector_all('.modal, .ui-modal, .ui-dialog, [role=dialog], .ui-overlay, .modal-backdrop')
