...
COOKIES = os.path.join(os.path.dirname(os.path.abspath('.')), 'fl_cookies.json')
...
with sync_playwright() as p:
    ...
    if btn:
        ...
        # Check all elements
        print('Iframes:', len(page.query_selector_all('iframe')))
        modals = page.query_selector_all('.modal, [role=dialog], .ui-modal')
        print('Modals:', len(modals))
        for m in modals:
            print('  Modal:', m.get_attribute('class'))
        
        # Check all elements with offer
